            ...
            <div >
              <div class="relative">
                ...
                <div class="absolute inset-y-0 right-0 flex items-center text-gray-500">
                  ...
                  >
                    <option>Немає початкового</option>
                    {categories.length > 0 && categories.map(category => (
                      <option key={category._id} value={category._id}>{category.name}</option>
                    ))}
                  </select>
                </div>
                <input type="text" id="example11" class="block w-[400px] rounded-md border border-slate-300 py-2.5 pl-8 pr-16 shadow-sm focus:border-primary-400 focus:ring focus:ring-primary-200 focus:ring-opacity-50 disabled:cursor-not-allowed disabled:bg-gray-50 disabled:text-gray-500" placeholder="Ім'я категорії"
                  value={name}
